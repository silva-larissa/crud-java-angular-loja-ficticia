...
public class NotaFiscalController {
	...
	public ResponseEntity<NotaFiscal> cadastrarNotaFiscal(@RequestBody @Validated NotaFiscal notaFiscal, UriComponentsBuilder uriBuilder){
		...
		for (ProdutoCompra produtoCompra : notaFiscal.getProdutosCompra()) { //ForEach para incluir o(s) produto(s) da compra na nota fiscal
			if (produtoCompra.getNotaFiscal()==null){
	...
	public ResponseEntity<NotaFiscal> atualizarNotaFiscal(@PathVariable Long id, @RequestBody NotaFiscal detalhesNotaFiscal) throws Exception {
		...
		atualizarNotaFiscal.setNumeroNota(detalhesNotaFiscal.getNumeroNota());
		atualizarNotaFiscal.setCliente(detalhesNotaFiscal.getCliente());
		atualizarNotaFiscal.setDataEmissao(detalhesNotaFiscal.getDataEmissao());

		BigDecimal novoValorNota = BigDecimal.ZERO;

		for (ProdutoCompra produtoCompra : detalhesNotaFiscal.getProdutosCompra()){
			if (produtoCompra.getNotaFiscal()==null){
				produtoCompra.setNotaFiscal(detalhesNotaFiscal);
			}
			novoValorNota = novoValorNota.add(produtoCompra.getValorParcial());
		}

		atualizarNotaFiscal.getProdutosCompra().clear();
		atualizarNotaFiscal.getProdutosCompra().addAll(detalhesNotaFiscal.getProdutosCompra());
		atualizarNotaFiscal.setValorNotaFiscal(novoValorNota);
